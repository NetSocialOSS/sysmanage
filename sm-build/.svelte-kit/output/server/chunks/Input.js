...
const Input = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div${add_attribute("class", inpClass, 0)}><label${add_attribute("for", id, 0)} class="block mb-1 font-medium text-gray-900 dark:text-gray-300">${escape(label)}</label>
    ...
    <textarea${add_attribute("tabindex", disabled ? -1 : 0, 0)} ${disabled ? "disabled" : ""}${add_attribute("aria-disabled", disabled, 0)}${add_attribute("minlength", minlength, 0)}${add_attribute("id", id, 0)} class="h-36 bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"${add_attribute("placeholder", placeholder, 0)} required>${value || ""}</textarea>
